...
const UserLogIn = () => {
  const { logedGuest, isGuest, setIsGuest, setLogedGuest } =
    useLogedGuestStore();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setSubmited(true);
    const getGuest = users.find(
      (users) => users.email == user.email && users.role == user.role
    );
    if (getGuest) {
      try {
        const isMatch = await bcrypt.compare(user.password, getGuest.password);
        if (isMatch) {
          ...
          localStorage.setItem(
            "myecommercelocalstorageadminguestkey",
            JSON.stringify(getGuest)
          );
          setLogedGuest(getGuest);
          setIsGuest(true);
    ...
    <div>
      ...
      <section className="px-3">
        <div className="max-w-md mx-auto p-8 bg-white rounded-lg shadow-md mt-16 border border-gray-200">
          <h2 className="text-2xl text-gray-600 text-center mb-6">Sign In</h2>
          {submited && !success ? (
